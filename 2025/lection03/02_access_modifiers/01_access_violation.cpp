...
public:
    ...
    void run(A* a) {
        ...
        cout << "Class B run: " << a->secret << endl;  // Чужое приватное поле нельзя читать
        a->secret = -1;                                // И писать в него тоже, разумеется, нельзя
    }
};
...
int main()
{
    ...
    // Это не скомпилится
    cout << "Main run: " << a.secret << endl;  // И main-у тоже нельзя читать приватные поля
    a.secret = -1;                             // И писать в них нельзя
